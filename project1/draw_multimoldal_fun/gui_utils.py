...
def plot_function_gui(function, x_range, y_range, cmap, ax):
    ax.clear()
...
def draw_gui(functions, colormaps):
    root = tk.Tk()
    ...
    toolbar_frame = tk.Frame(root)  # New frame for the toolbar
    toolbar_frame.grid(row=2, column=0, sticky='nsew')  # Grid layout
    toolbar = NavigationToolbar2Tk(canvas, toolbar_frame)  # Use new frame
    toolbar.update()
    ...
    def plot_button_callback():
        ...
        cmap = cmap_var.get()
        plot_function_gui(functions[function_name], x_range, y_range, cmap, ax)
        canvas.draw()
